...
Before running the placement matrix, sweep all-cloud prompt variants to find the
best stable C1/C2 baseline:

```bash
python3 scripts/run_cti_prompt_sensitivity.py \
  --aces-root ~/ACESEvals \
  --dataset-size 25 \
  --limit 25 \
  --timeout-s 60 \
...
The sensitivity script records timeout/error rows instead of aborting, so use
`ok_n`, `error_n`, and `ok_proxy_quality` to separate model stability from
quality on completed samples.
...
given a platform-scoped dataset-wide MITRE technique catalog with compact
technique-name aliases and C2 is given a dataset-wide data-source catalog, analogous to
